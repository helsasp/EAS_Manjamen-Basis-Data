...
MySQL - 10.4.32-MariaDB : Database - db_uas_5025221154
*********************************************************************
*/
...
CREATE DATABASE /*!32312 IF NOT EXISTS*/`db_uas_5025221154` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci */;

USE `db_uas_5025221154`;
...
(1,'Beverages','Minuman'),
(2,'Condiments','Minuman'),
(3,'Confections','Minuman'),
(4,'Dairy Products','Minuman'),
(5,'Grains/Cereals','Minuman'),
(6,'Meat/Poultry','Minuman'),
(7,'Produce','Minuman'),
(8,'Seafood','Minuman'),
(9,'Minuman','Minuman sehat'),
(10,'Makanan','Makanan enak'),
(13,'Maks',NULL);
...
(11,'Putri','Helsa','2004-06-03',NULL,NULL);

/*Table structure for table `employees_mahasiswa` */

DROP TABLE IF EXISTS `employees_mahasiswa`;

CREATE TABLE `employees_mahasiswa` (
  `EmployeeID` int(11) DEFAULT NULL,
  `NRP` char(15) DEFAULT NULL,
  `usia_in_sec` bigint(20) DEFAULT 0,
  KEY `EmployeeID` (`EmployeeID`),
  CONSTRAINT `fk_employees_mahasiswa` FOREIGN KEY (`EmployeeID`) REFERENCES `employees` (`EmployeeID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;

/*Data for the table `employees_mahasiswa` */

insert  into `employees_mahasiswa`(`EmployeeID`,`NRP`,`usia_in_sec`) values 
(11,'5025221154',633201452);
...
insert  into `products`(`ProductID`,`ProductName`,`CategoryID`,`Unit`,`Price`) values 
...
(22,'Gustaf\'s Knäckebröd',5,'24 - 500 g pkgs.',21),
(23,'Tunnbröd',5,'12 - 250 g pkgs.',9),
(24,'Guaraná Fantástica',1,'12 - 355 ml cans',4.5),
(25,'NuNuCa Nuß-Nougat-Creme',3,'20 - 450 g glasses',14),
(26,'Gumbär Gummibärchen',3,'100 - 250 g bags',31.23),
(27,'Schoggi Schokolade',3,'100 - 100 g pieces',43.9),
(28,'Rössle Sauerkraut',7,'25 - 825 g cans',45.6),
(29,'Thüringer Rostbratwurst',6,'50 bags x 30 sausgs.',123.79),
...
(54,'Tourtière',6,'16 pies',7.45),
(55,'Pâté chinois',6,'24 boxes x 2 pies',24),
...
(61,'Sirop d\'érable',2,'24 - 500 ml bottles',28.5),
(62,'Tarte au sucre',3,'48 pies',49.3),
(63,'Vegie-spread',2,'15 - 625 g jars',43.9),
(64,'Wimmers gute Semmelknödel',5,'20 bags x 4 pieces',33.25),
...
(71,'Fløtemysost',4,'10 - 500 g pkgs.',21.5),
(72,'Mozzarella di Giovanni',4,'24 - 200 g pkgs.',34.8),
(73,'Röd Kaviar',8,'24 - 150 g jars',15),
(74,'Longlife Tofu',7,'5 kg pkg.',10),
(75,'Rhönbräu Klosterbier',1,'24 - 0.5 l bottles',7.75),
(76,'Lakkalikööri',1,'500 ml',18),
(77,'Original Frankfurter grüne Soße',2,'12 boxes',13),
(78,'Chaisee',1,NULL,108),
(79,'Concealer',8,NULL,55),
(80,'Chaisue',13,NULL,56),
(81,'Chaisuue',13,NULL,56),
(82,'Chaeee',1,NULL,108),
(83,'Chaeyee',10,NULL,108);
...
/* Event structure for event `ev_update_usia_in_sec` */

/*!50106 DROP EVENT IF EXISTS `ev_update_usia_in_sec`*/;

DELIMITER $$

/*!50106 CREATE DEFINER=`root`@`localhost` EVENT `ev_update_usia_in_sec` ON SCHEDULE EVERY 1 SECOND STARTS '2024-06-26 16:35:51' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    UPDATE employees_mahasiswa
    SET usia_in_sec = usia_in_sec + 1
    WHERE NRP = '5025221154';
END */$$
DELIMITER ;

/* Function  structure for function  `fn_usia_in_sec` */

/*!50003 DROP FUNCTION IF EXISTS `fn_usia_in_sec` */;
DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` FUNCTION `fn_usia_in_sec`(tglLahir DATETIME
) RETURNS int(11)
BEGIN
    DECLARE usia_in_sec INT;

    -- Hitung usia dalam detik
    SET usia_in_sec = TIMESTAMPDIFF(SECOND, tglLahir, NOW());

    RETURN usia_in_sec;
END */$$
DELIMITER ;

/* Procedure structure for procedure `sp_categories_add_or_update` */

/*!50003 DROP PROCEDURE IF EXISTS  `sp_categories_add_or_update` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_categories_add_or_update`(
...
BEGIN
    ...
    -- Cek apakah CategoryName sudah ada
    SELECT CategoryID
    INTO v_CategoryID
    FROM categories
    WHERE CategoryName = p_CategoryName;

    -- Jika CategoryName tidak ditemukan, tambahkan kategori baru
    IF v_CategoryID IS NULL THEN
        INSERT INTO categories (CategoryName, Description)
        VALUES (p_CategoryName, p_Description);
        
        -- Output pesan dan status
        SET v_Message = '0, "Kategori belum ada. Penambahan SUKSES"';
    ELSE
        -- Jika CategoryName ditemukan, update deskripsi
        UPDATE categories
        SET Description = p_Description
        WHERE CategoryID = v_CategoryID;
        
        -- Output pesan dan status
        SET v_Message = '1, "Kategori sudah ada. Pembaruan deskripsi SUKSES"';
    END IF;

    -- Kembalikan pesan sebagai hasil
    SELECT v_Message AS ResultMessage;
END */$$
DELIMITER ;

/* Procedure structure for procedure `sp_products_add` */

/*!50003 DROP PROCEDURE IF EXISTS  `sp_products_add` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_products_add`(
...
BEGIN
    ...
    SELECT COUNT(*) INTO categoryCheck FROM categories WHERE CategoryID = p_CategoryID;

    IF categoryCheck = 0 THEN
        SELECT -2 AS Result, CONCAT("penambahan GAGAL. CategoryID=", p_CategoryID, " tidak ditemukan") AS Message;
    ELSE
        -- Check if productName already exists
        IF EXISTS (SELECT * FROM products WHERE BINARY ProductName = p_productName) THEN
            SELECT -1 AS Result, "penambahan GAGAL. Nama produk sudah ada" AS Message;
        ELSE
            -- Insert new product
            INSERT INTO products (ProductName, CategoryID, Price) VALUES (p_productName, p_CategoryID, p_price);
            SELECT 0 AS Result, "penambahan Produk SUKSES" AS Message;
        END IF;
    END IF;
END */$$
DELIMITER ;

/* Procedure structure for procedure `sp_products_add_ext` */

/*!50003 DROP PROCEDURE IF EXISTS  `sp_products_add_ext` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_products_add_ext`(
    IN p_productName VARCHAR(255),
    IN p_CategoryName VARCHAR(255),
    IN p_price DOUBLE
)
BEGIN
    ...
    BEGIN
        ...
        ROLLBACK;
        SELECT -99 AS Result, "GAGAL. Kesalahan tidak diketahui. Rollback" AS Message;
    END;
    ...
    FROM products
    WHERE BINARY ProductName = p_productName;

    IF v_productExists > 0 THEN
        ROLLBACK;
        SELECT -1 AS Result, CONCAT("penambahan GAGAL. Nama produk = ", p_productName, " sudah ada") AS Message;
    ELSE
        ...
        IF v_categoryID IS NULL THEN
            -- CategoryName does not exist, so insert it
            INSERT INTO categories (CategoryName, Description) VALUES (p_CategoryName, NULL);
            SET v_categoryID = LAST_INSERT_ID();
            SELECT 1 AS Result, CONCAT("penambahan SUKSES. Kategori ", p_CategoryName, " telah ditambahkan") AS Message;
        END IF;

        -- Insert new product
        INSERT INTO products (ProductName, CategoryID, Price) VALUES (p_productName, v_categoryID, p_price);
        COMMIT;
        SELECT 0 AS Result, CONCAT("penambahan SUKSES. Terinput dg CategoryID = ", v_categoryID) AS Message;
    END IF;
END */$$
DELIMITER ;
